...
if [ -n "${HARNESS_MUTATION_THRESHOLD:-}" ] && [ -f stryker.config.mjs ]; then
  BREAK_AT="$(grep -oE 'break:[[:space:]]*[0-9]+' stryker.config.mjs | grep -oE '[0-9]+' | head -1)"
  if [ -n "$BREAK_AT" ] && [ "$BREAK_AT" != "$HARNESS_MUTATION_THRESHOLD" ]; then
    ...
    echo "[stryker-runner]         pero stryker.config.mjs tiene thresholds.break=$BREAK_AT." >&2
...
ARGS=(run --concurrency "${HARNESS_MUTATION_CONCURRENCY:-2}")
