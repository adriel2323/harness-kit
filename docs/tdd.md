...
> followed by code (TDD)?" — La respuesta de este arnés: **single test
...
- **ROJO** — el test deriva del siguiente escenario `@s` del `.feature`.
  Verifícalo fallando de verdad (corre `HARNESS_TEST_CMD`). Un test que
...
Refactor. Un escenario con varias aristas (p. ej. "entrada vacía → 0" y
"tres elementos → 3") puede necesitar dos ciclos para forzar la
...
```markdown
## Trazabilidad
- @s1 (entrada vacía → 0) → test_count_entrada_vacia
- @s2 (tres elementos → 3) → test_count_varios
